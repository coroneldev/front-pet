  ...
  <div class="hello">
    ...
    <q-card flat bordered>
      <q-card-section class="bg-grey-2 my-card">
        <div class="text-h5 text-center">Áreas</div>
      </q-card-section>
      ...
      <q-card-section class="bg-grey-2 my-card">
        ...
        <q-btn color="primary" label="Nuevo" icon-right="add" to="/areas/create" />
          ...
          <q-markup-table flat bordered>
            ...
            <tbody>
              <tr v-for="(area, index) in areas" :key="index">
                ...
                <td>
                  <q-btn round color="white" text-color="black" icon="mode_edit" class="q-ml-sm" :to="'/areas/edit/'+area.id" title="Editar"></q-btn>
                  <q-btn round color="white" text-color="black" icon="delete" class="q-ml-sm" @click="onModal(area.id)" title="Eliminar"></q-btn>
                </td>
              </tr>
            </tbody>
          </q-markup-table>
...
export default defineComponent({
  ...
  methods: {
    ...
    onEliminar(){ 
      Loading.show({message: "Cargando..."});
      AreaService.delete(this.idEliminar)
        .then((response: any) => {
          if(response.data.status == true) {
            this.alert = false;
            this.idEliminar = 0;
