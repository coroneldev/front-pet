  ...
  <div class="hello">
    ...
    <q-card flat bordered>
      <q-card-section class="bg-grey-2 my-card">
        <div class="text-h5 text-center">Editar Área</div>
      </q-card-section>
      ...
      <q-card-section class="bg-grey-2 my-card">
        ...
        <div class="row q-mt-md">
          <div class="col-12 col-md-4 q-pa-xs">
            <q-input outlined v-model="area.nombre" label="Nombre *" counter maxlength="50" />
          </div>
          <div class="col-12 col-md-8 q-pa-xs">
            <q-input outlined v-model="area.descripcion" label="Descripción" counter maxlength="100" />
          </div>
        </div>

        <div class="row q-mt-md">
          <q-btn color="primary" label="Guardar" @click="onModal()" />
          <q-btn color="white" text-color="black" label="Cancelar" class="q-ml-sm" to="/areas" />
        </div>
...
export default defineComponent({
  ...
  methods: {
    ...
    getData() {
      ...
      AreaService.get(this.$route.params.id)
        .then((response: any) => {
    ...
    onGuardar() {
      ...
      AreaService.update(this.$route.params.id, data)
        .then((response: any) => {
          if(response.data.status == true) {
            this.area = response.data.data;
            this.alert = false;
